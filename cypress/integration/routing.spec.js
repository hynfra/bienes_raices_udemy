/// <references types="cypress" /> // agrega el json de cypress

describe('Prueba la navegacion y routing del header y footer',() => {
    it('prueba la navegacion de header', () => {
        cy.visit('/');

        cy.get('[data-cy="navegacion-header"]').should('exist');
        cy.get('[data-cy="navegacion-header"]').find('a').should('have.length',4);
        cy.get('[data-cy="navegacion-header"]').find('a').should('not.have.length',6);
        
        // revisar que los enlaces son correctos

        //si hay varios elementos del mismo se usa eq para que los devuelva como arreglo (parten del 0 en adelante)
        // en invoke se accede a un atributo con attr y luego el nombre de este (en este caso href)
        cy.get('[data-cy="navegacion-header"]').find('a').eq(0).invoke('text').should('equal','Nosotros')
        
        cy.get('[data-cy="navegacion-header"]').find('a').eq(0).invoke('attr','href').should('equal','nosotros.php')
        cy.get('[data-cy="navegacion-header"]').find('a').eq(1).invoke('attr','href').should('equal','anuncios.php')
        cy.get('[data-cy="navegacion-header"]').find('a').eq(1).invoke('text').should('equal','Anuncios')
    
        cy.get('[data-cy="navegacion-header"]').find('a').eq(2).invoke('attr','href').should('equal','blog.php')
        cy.get('[data-cy="navegacion-header"]').find('a').eq(2).invoke('text').should('equal','Blog')
    
        cy.get('[data-cy="navegacion-header"]').find('a').eq(3).invoke('text').should('equal','Contacto')
        cy.get('[data-cy="navegacion-header"]').find('a').eq(3).invoke('attr','href').should('equal','contacto.php')
    });
    

    it('prueba la navegacion de footer', () => {
        cy.visit('/');

        cy.get('[data-cy="navegacion-footer"]').should('exist');
        // should('have.prop','class') indica que deberia tener una propiedad que sea class
        cy.get('[data-cy="navegacion-footer"]').should('have.prop','class').should('equal', 'navegacion');
        cy.get('[data-cy="navegacion-footer"]').find('a').should('have.length',4);
        cy.get('[data-cy="navegacion-footer"]').find('a').should('not.have.length',6);
        
        // revisar que los enlaces son correctos

        //si hay varios elementos del mismo se usa eq para que los devuelva como arreglo (parten del 0 en adelante)
        // en invoke se accede a un atributo con attr y luego el nombre de este (en este caso href)
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(0).invoke('attr','href').should('equal','nosotros.php')
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(0).invoke('text').should('equal','Nosotros')
    
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(1).invoke('attr','href').should('equal','anuncios.php')
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(1).invoke('text').should('equal','Anuncios')
    
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(2).invoke('attr','href').should('equal','blog.php')
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(2).invoke('text').should('equal','Blog')
    
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(3).invoke('attr','href').should('equal','contacto.php')
        cy.get('[data-cy="navegacion-footer"]').find('a').eq(3).invoke('text').should('equal','Contacto')
    });

})

Cypress.on('uncaught:exception', (err, runnable) => {
    console.log(err);
    return false;
  })